...
- Cached diff supports staged rename/copy detection with `-M[<n>]`,
  ...
  cached copy slices.
  Empty long-form thresholds such as `--find-renames=` and
...
- Default worktree diff already performs checked exact rename detection for
  ...
  The same Git-compatible default-threshold behavior is checked there for
  empty long-form options such as `--find-renames=` and `--find-copies=`.
  Covered worktree parser behavior also accepts split `-l <n>` forms like
  Git rather than requiring only `-l<n>`. Covered default worktree parser
...
That same subdirectory coverage also includes NUL-delimited stdin and
...
of `core.ignorecase`.
Exact glob magic pathspec `:(glob)camel.txt` is also checked on the same
baseline: `status`, `diff`, `ls-files`, `log`, and `show` keep the same
