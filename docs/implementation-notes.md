...
- Git-compatible behavior: `rit stash show --stat-width=<n>`,
  `--stat-name-width=<n>`, `--stat-graph-width=<n>`, and `--stat-count=<n>`
  are accepted for checked simple stat output and reject non-numeric values
  with Git-compatible errors.
