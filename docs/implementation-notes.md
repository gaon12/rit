...
- 2026-05-18 stash show output file slice checked Git 2.52.0.windows.1
  ...
  file bytes.
- 2026-05-18 stash show path prefix slice checked Git 2.52.0.windows.1 with
...
- 2026-05-18 stash show stat width/count passthrough slice checked
  Git 2.52.0.windows.1 with `git --version`, direct Git comparisons for
  `stash show --stat[=<width>[,<name-width>[,<count>]]]`,
  `stash show --stat-width=<n>`, `stash show --stat-name-width=<n>`,
  `stash show --stat-graph-width=<n>`, `stash show --stat-count=<n>`, and
  matching `stash show --patch-with-stat` forms on simple stat output, plus
...
- 2026-05-17 stash show diff algorithm passthrough slice checked
  ...
  `stash show --patch --minimal`, `--patience`, `--histogram`, and
  `--diff-algorithm=<algorithm>` on simple text stash changes, including
  Git-compatible invalid diff-algorithm errors.
...
- 2026-05-18 stash show intent-to-add passthrough slice checked
  Git 2.52.0.windows.1 with `git --version`, `git stash show -h`,
  `git diff -h`, and direct Git comparisons for `stash show --patch` with
  `--ita-invisible-in-index` and `--ita-visible-in-index` on simple text
  stash changes.
- 2026-05-17 stash show rename/copy passthrough slice checked `git stash -h`,
  `git diff -h`, and direct Git comparisons for `stash show --patch` with
  `--binary`, `--no-renames`, `--find-renames`, `-M`, `--find-copies`, `-C`,
  and `--find-copies-harder` forms on simple text stash changes.
...
- 2026-05-22 zero diff.renameLimit config slice checked `git diff -h` and
  direct Git-vs-rit comparisons for cached/worktree `diff -M` plus
  cached/worktree `diff -C` when `diff.renameLimit=0`, confirming that config
  `0` keeps rename/copy detection unlimited like explicit `-l0`.
...
- 2026-05-23 hard-copy no-renames slice checked Git 2.54.0.windows.1 with
  ...
  `--find-copies-harder` copy detection, with or without `-C`, stays active
  in cached/worktree diff even when `--no-renames` appears before or after
  it.
...
- 2026-05-17 diff.renames default/config slice checked Git 2.52.0.windows.1
  ...
  and exit code.
- 2026-05-24 diff.renames=copies copy-slice follow-up checked Git
  2.54.0.windows.1 with `git diff -h` and direct Git-vs-rit comparisons for
  cached/worktree default rename, copy, and hard-copy slices, confirming that
  the checked config matches Git without forcing extra copy promotion there.
- 2026-05-24 diff.renames=false explicit hard-copy follow-up checked the same
  Git baseline with direct Git-vs-rit comparisons showing that checked
  `--find-copies-harder`, with or without `-C`, still overrides
  `diff.renames=false` on the covered cached/worktree hard-copy slices.
- 2026-05-24 diff.renames=false explicit copy follow-up checked the same Git
...
- 2026-05-24 pathspec-file-nul option-order slice checked `git add -h`,
  `git restore -h`, and `git reset -h` plus direct Git-vs-rit comparisons
  showing that `--pathspec-file-nul` still applies when it appears before
  `--pathspec-from-file`, including `--pathspec-from-file=-`.
...
- 2026-05-24 no pathspec-from-file active-stdin follow-up checked the same
  Git baseline with direct Git-vs-rit comparisons showing that
...
- 2026-05-24 no pathspec-from-file later-selection follow-up checked the same
  ...
  `reset`.
- 2026-05-25 no pathspec-from-file later-stdin follow-up checked the same Git
  baseline with direct Git-vs-rit comparisons showing that an earlier
...
- 2026-05-24 subdirectory stdin pathspec follow-up checked the same Git
  ...
  invocation directory unless they use top magic for `add`, `restore`, and
  `reset`.
- 2026-05-24 subdirectory stdin NUL pathspec follow-up checked the same Git
  baseline with direct Git-vs-rit comparisons showing that NUL-delimited
  pathspec entries read from stdin inside a subdirectory are likewise
...
- 2026-05-18 init no-template slice checked Git 2.52.0.windows.1 with
  `git --version`, `git init -h`, and direct Git comparisons for
  `init -q --no-template` and template-selection resets.
...
- `--pathspec-file-nul` is also honored when it appears before
...
- Stdin-delivered pathspec entries inside a subdirectory follow that same
  ...
  and `reset`.
- NUL-delimited stdin pathspec entries inside a subdirectory follow that same
  relative/top-magic resolution model, matching Git for `add`, `restore`,
  and `reset`.
...
- Supported options: `-q`, `--quiet`, `--no-quiet`, `--bare`, `--no-bare`,
  `--no-template`, template selection followed by `--no-template`,
  ...
  directory.
- Unsupported options: applying templates, separate git dir, non-SHA-1 object
  formats, non-files ref formats, shared repositories.
...
- Supported options: default patch output for small text files, `-p`, `-u`,
  ...
  re-enables rename detection, and checked `--find-copies-harder` copy
  detection remains active even when `--no-renames` appears before or after
  it, with or without `-C`. Plain checked `-C` copy detection follows the same option-order idea:
...
- Git-compatible behavior:
  ...
  `--stat-count=<n>` are accepted for checked simple stat output and reject
  invalid numeric values with Git-compatible errors.
...
- Git-compatible behavior: `rit stash save [-q] [-k|-S|-u|-a] [<message>]`
  follows the same tracked/untracked/keep-index/staged implementations as
  push, accepts a legacy positional message, and matches Git's saved/no-change
  output for the checked scope.
...
- Git-compatible behavior: `rit stash show` diffs the selected stash commit
  ...
  in the checked tracked and untracked patch scopes. `--diff-filter=<letters>`
  ...
  `--minimal`, `--patience`, `--histogram`, `--diff-algorithm=<algorithm>`,
  and `--anchored=<text>` are accepted as diff algorithm passthrough options
  for checked simple text stash patches where the current diff engine produces
  Git's same patch shape, including Git-compatible invalid diff-algorithm
  errors. `-w`, `--ignore-all-space`, `-b`, `--ignore-space-change`,
  `--ignore-space-at-eol`,
  `--ignore-cr-at-eol`, `--ignore-blank-lines`, `--indent-heuristic`,
  `--no-indent-heuristic`, and `--ws-error-highlight=<kind>` are accepted as
  ...
  passthrough option in that same checked scope. `--ita-invisible-in-index`
  and `--ita-visible-in-index` are accepted as intent-to-add passthrough
  options for the same checked simple text stash patch scope.
...
- 2026-05-25 repo-root magic case-sensitive lookup slice checked the same Git
  baseline with direct Git-vs-rit comparisons for read-only `status`, `diff`,
  `ls-files`, `ls-tree`, `log`, and `show`, plus write `add`, `reset`, and
  `restore`, using repo-root tracked pathspec `:(top)camel.txt` against
  committed `Camel.txt`. In that baseline, read-only repo-root lookup stayed
  case-sensitive regardless of `core.ignorecase`; `add` still honored
  `core.ignorecase`, `reset` kept the tracked-path no-op behavior, and
  `restore` rejected the mismatched-case repo-root pathspec.
- 2026-05-25 root shorthand case-sensitive lookup slice checked the same Git
  baseline with direct Git-vs-rit comparisons for read-only `status`, `diff`,
  `ls-files`, `ls-tree`, `log`, and `show`, plus write `add`, `reset`, and
  `restore`, using root shorthand tracked pathspec `:/camel.txt` against
  committed `Camel.txt`. In that baseline, read-only root shorthand lookup
