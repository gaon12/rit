...
This repository currently implements the initial workspace skeleton, the `rit` CLI entry point, repository discovery, loose object reading/writing, packed object reading for non-delta objects, index reading/writing, local refs, simple revision resolution, and basic `help` / `version` / `init` / `rev-parse` / `cat-file` / `ls-tree` / `show` / `status --porcelain=v1` / `diff --name-only` / `diff --stat` / `log` / `add` / `commit` / `branch` / `tag` / `restore` / `reset` / `checkout` / `switch` commands.
