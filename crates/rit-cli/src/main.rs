...
fn run(
    ...
    match args.as_slice() {
        ...
        [command, rest @ ..] if command == "stash" => stash::stash_command(rest, stdout, stderr),
        ...
        [command, rest @ ..] if command == "workspace" => {
            workspace::workspace_command(rest, stdout, stderr)
        }
...
pub(crate) fn parse_similarity_option(
    option: &str,
    short_prefix: &str,
    long_prefix: &str,
) -> Result<u32, String> {
...
pub(crate) fn is_valid_break_rewrites_option(option: &str) -> bool {
...
pub(crate) fn parse_rename_limit_option(option: &str) -> Result<usize, String> {
...
fn show_command(
    ...
    let Some((revision, pathspec_args, patch_enabled)) = parse_show_args(args, stderr)? else {
        return Ok(ExitCode::from(129));
    };
    let repository = match discover_repository(stderr)? {
        Some(repository) => repository,
...
fn parse_show_args(
    ...
    let mut pathspecs = Vec::new();
    let mut after_separator = false;
    let mut patch_enabled = true;

    for arg in args {
        match arg.as_str() {
            "--" if !after_separator => after_separator = true,
            "--patch" | "-p" if !after_separator => patch_enabled = true,
            "--no-patch" | "-s" if !after_separator => patch_enabled = false,
            unsupported if unsupported.starts_with('-') && !after_separator => {
...
fn parse_merge_args(
    ...
    while index < args.len() {
        ...
        if arg == "--plan" {
            ...
            index += 1;
            let Some(value) = args.get(index) else {
                writeln!(stderr, "rit: merge strategy requires a value")?;
                return Ok(None);
            };
            let Some(parsed_strategy) = parse_merge_strategy(value, stderr)? else {
                return Ok(None);
            };
...
fn parse_merge_strategy(
    value: &str,
    stderr: &mut dyn Write,
) -> io::Result<Option<rit_core::MergeStrategy>> {
    match value {
        "ort" | "recursive" => Ok(Some(rit_core::MergeStrategy::Default)),
        "ours" => Ok(Some(rit_core::MergeStrategy::Ours)),
        "" => {
            writeln!(stderr, "rit: merge strategy requires a value")?;
            Ok(None)
        }
...
pub(crate) fn write_command_error(
    stderr: &mut dyn Write,
    error: rit_core::RitError,
) -> io::Result<ExitCode> {
    if let rit_core::RitError::InvalidInput { message } = &error {
...
pub(crate) fn write_pathspec_file_nul_requires_file(stderr: &mut dyn Write) -> io::Result<()> {
    writeln!(
        stderr,
...
pub(crate) fn write_pathspec_file_cannot_mix_with_args(stderr: &mut dyn Write) -> io::Result<()> {
    writeln!(
        stderr,
...
pub(crate) fn pathspec_from_file_missing_value(
    args: &[String],
    index: usize,
...
pub(crate) fn discover_repository(
    stderr: &mut dyn Write,
) -> io::Result<Option<rit_core::Repository>> {
