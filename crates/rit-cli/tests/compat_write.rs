...
fn empty_pathspec_from_file_value_can_mix_with_args_like_git() {
    for command in ["add", "restore", "reset"] {
        let fixture = LocalWriteFixture::new(
            &format!("{command}-empty-pathspec-from-file-with-args"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(
        ...
        let workspace = temp_path(&format!(
            "{command}-empty-pathspec-from-file-with-args-compare"
        ));
        let git_repo = workspace.join("git");
        let rit_repo = workspace.join("rit");
        copy_directory(fixture.path(), &git_repo);
        ...
        let git = run_command_allow_failure(
            &command_words(
                "git",
                [command, "--pathspec-from-file=", "nested/tracked.txt"],
            ),
            &git_repo,
        );
        let rit = run_command_allow_failure(
...
#[test]
fn empty_pathspec_from_file_value_with_nul_is_rejected_like_git() {
    for command in ["add", "restore", "reset"] {
        let fixture = LocalWriteFixture::new(
            &format!("{command}-empty-pathspec-from-file-with-nul"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(
        ...
        let workspace = temp_path(&format!(
            "{command}-empty-pathspec-from-file-with-nul-compare"
        ));
        let git_repo = workspace.join("git");
        let rit_repo = workspace.join("rit");
        copy_directory(fixture.path(), &git_repo);
...
#[test]
fn repeated_pathspec_from_file_uses_later_text_file_after_nul_stdin_like_git() {
    for command in ["add", "restore", "reset"] {
        let fixture = LocalWriteFixture::new(
            &format!("{command}-repeated-pathspec-from-file-later-text-file-after-nul-stdin"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(fixture.path().join("other.txt"), "base\n")
...
#[test]
fn repeated_pathspec_from_file_rejects_later_text_stdin_after_nul_mode_like_git() {
    for command in ["add", "restore", "reset"] {
        let fixture = LocalWriteFixture::new(
            &format!("{command}-repeated-pathspec-from-file-later-text-stdin-after-nul"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(fixture.path().join("other.txt"), "base\n")
...
#[test]
fn repeated_pathspec_from_file_rejects_later_text_stdin_after_nul_stdin_like_git() {
    for command in ["add", "restore", "reset"] {
        let fixture = LocalWriteFixture::new(
            &format!("{command}-repeated-pathspec-from-file-later-text-stdin-after-nul-stdin"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(fixture.path().join("other.txt"), "base\n")
...
#[test]
fn text_utf8_bom_pathspec_file_matches_git_behavior() {
    for command in ["add", "restore", "reset"] {
        let fixture = LocalWriteFixture::new(
            &format!("{command}-text-utf8-bom-pathspec-file"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(
        ...
        bytes.extend_from_slice(b"nested/tracked.txt\n");
        fs::write(fixture.path().join("pathspecs.bin"), bytes)
            .expect("pathspec file should be written");
        if command == "reset" {
            run_git(fixture.path(), ["add", "nested/tracked.txt"]);
        }

        let workspace = temp_path(&format!("{command}-text-utf8-bom-pathspec-file-compare"));
        let git_repo = workspace.join("git");
        let rit_repo = workspace.join("rit");
        copy_directory(fixture.path(), &git_repo);
        copy_directory(fixture.path(), &rit_repo);
...
fn add_exclude_magic_pathspec_matches_git_status() {
    for (name, alias) in [
        ("short", ":!nested/new.txt"),
        ("caret", ":^nested/new.txt"),
        ("long", ":(exclude)nested/new.txt"),
    ] {
        let fixture = LocalWriteFixture::new(
            &format!("add-exclude-magic-{name}"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        ...
        let outcome = compare_after_command(
            fixture.path(),
            command_words("git", ["add", "*.txt", alias]),
            command_words(rit_binary(), ["add", "*.txt", alias]),
        );
...
#[test]
fn add_icase_glob_wildcard_pathspec_matches_git_status() {
    for core_ignorecase in [false, true] {
        let fixture = case_pathspec_fixture(
            &format!("add-icase-glob-wildcard-{core_ignorecase}"),
            core_ignorecase,
            false,
        );
        ...
        let outcome = compare_after_command(
            &fixture,
            command_words("git", ["add", ":(icase,glob)camel*"]),
            command_words(rit_binary(), ["add", ":(icase,glob)camel*"]),
        );
...
#[test]
fn restore_icase_glob_wildcard_pathspec_matches_git_status_and_files() {
    for core_ignorecase in [false, true] {
        let fixture = case_pathspec_fixture(
            &format!("restore-icase-glob-wildcard-{core_ignorecase}"),
            core_ignorecase,
            false,
        );
        ...
        let outcome = compare_after_command(
            &fixture,
            command_words("git", ["restore", ":(icase,glob)camel*"]),
            command_words(rit_binary(), ["restore", ":(icase,glob)camel*"]),
        );
...
fn restore_exclude_magic_pathspec_matches_git_status_and_files() {
    for (name, alias) in [
        ("short", ":!nested/skip.txt"),
        ("caret", ":^nested/skip.txt"),
        ("long", ":(exclude)nested/skip.txt"),
    ] {
        let fixture = LocalWriteFixture::new(
            &format!("restore-exclude-magic-{name}"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(fixture.path().join("top.txt"), "top base\n")
        ...
        let outcome = compare_after_command(
            fixture.path(),
            command_words("git", ["restore", "*.txt", "nested/*.txt", alias]),
            command_words(rit_binary(), ["restore", "*.txt", "nested/*.txt", alias]),
        );
        ...
        for relative_path in ["top.txt", "nested/tracked.txt", "nested/skip.txt"] {
            assert_eq!(
                fs::read_to_string(outcome.git_repo.join(relative_path))
                    .expect("git file should read"),
                fs::read_to_string(outcome.rit_repo.join(relative_path))
                    .expect("rit file should read")
            );
        }
    }
...
#[test]
fn reset_icase_glob_wildcard_pathspec_matches_git_status() {
    for core_ignorecase in [false, true] {
        let fixture = case_pathspec_fixture(
            &format!("reset-icase-glob-wildcard-{core_ignorecase}"),
            core_ignorecase,
            true,
        );
        ...
        let outcome = compare_after_command(
            &fixture,
            command_words("git", ["reset", ":(icase,glob)camel*"]),
            command_words(rit_binary(), ["reset", ":(icase,glob)camel*"]),
        );
...
fn reset_exclude_magic_pathspec_matches_git_status() {
    for (name, alias) in [
        ...
        let fixture = LocalWriteFixture::new(
            &format!("reset-exclude-magic-{name}"),
            LocalWriteFixtureKind::NestedTracked,
        )
        .expect("fixture should build");
        fs::write(fixture.path().join("top.txt"), "top base\n")
        ...
        let outcome = compare_after_command(
            fixture.path(),
            command_words("git", ["reset", "*.txt", "nested/*.txt", alias]),
            command_words(rit_binary(), ["reset", "*.txt", "nested/*.txt", alias]),
        );
